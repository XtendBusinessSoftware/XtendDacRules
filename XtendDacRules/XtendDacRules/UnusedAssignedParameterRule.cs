...
namespace Xtend.Dac.Rules
{
    [XtendExportCodeAnalysisRuleAttribute(
        UnusedAssignedParameterRule.RuleId,
        RuleConstants.UnusedAssignedParameter_RuleName,              // ID used to look up the display name inside the resources file
        RuleConstants.UnusedAssignedParameter_ProblemDescription,    // ID used to look up the description inside the resources file
        Category = RuleConstants.CategoryDesign,            // Rule category (e.g. "Design", "Naming")
        RuleScope = SqlRuleScope.Element)]                  // This rule targets specific elements rather than the whole model

    public sealed class UnusedAssignedParameterRule : XtendSqlProcedureAnalysisRule
    {
        /// <summary>
        /// This rule= will be grouped by "Xtend.Dac.Rules.Naming", with the rule
        /// shown as "XR0015: Parameter is assigned but its value is never used"
        /// </summary>
        public const string RuleId = "Xtend.Rules.Data.XR0015";

        public UnusedAssignedParameterRule()
        {
        ...
        public override IList<SqlRuleProblem> Analyze(XtendSqlRuleExecutionContext context)
        {
            ...
            UnusedVariableVisitor visitor = new UnusedVariableVisitor(true, true);
