...
namespace Xtend.Dac.Rules
{
    [XtendExportCodeAnalysisRuleAttribute(
        CursorMissingDeallocateRule.RuleId,
        RuleConstants.CursorMissingDeallocate_RuleName,              // ID used to look up the display name inside the resources file
        RuleConstants.CursorMissingDeallocate_ProblemDescription,    // ID used to look up the description inside the resources file
        Category = RuleConstants.CategoryDesign,            // Rule category (e.g. "Design", "Naming")
        RuleScope = SqlRuleScope.Element)]                  // This rule targets specific elements rather than the whole model
    public sealed class CursorMissingDeallocateRule : SqlCodeAnalysisRule
    {
        /// <summary>
        ...
        public CursorMissingDeallocateRule()
        {
        ...
        public override IList<SqlRuleProblem> Analyze(SqlRuleExecutionContext ruleExecutionContext)
        {
            ...
            {
                CursorVisitor visitor = new CursorVisitor();
                fragment.Accept(visitor);
                foreach (var element in visitor.DeclareCursorStatements)
                {
                    var cursor = element.Value;
                    if (cursor == null)
                        continue;
                    string cursorName = cursor.Name.Value;
                    var deallocate = visitor.GetDeallocateCursorStatement(element.Key);
                    if (deallocate == null)
                    {
                        SqlRuleProblem problem = new SqlRuleProblem(
                                                    String.Format(
                                                        CultureInfo.CurrentCulture,
                                                        ruleDescriptor.DisplayDescription, "Missing",
                                                        cursorName,
                                                        elementName),
                                                    modelElement,
                                                    cursor);
                ...
                foreach (var element in visitor.DeallocateCursorStatements)
                {
                    var declare = visitor.GetDeclareCursorStatement(element.Key);
                    if (declare == null)
                    {
                        SqlRuleProblem problem = new SqlRuleProblem(
                                                    String.Format(
                                                        CultureInfo.CurrentCulture,
                                                        ruleDescriptor.DisplayDescription, "Invalid",
                                                        element.Key,
                                                        elementName),
                                                    modelElement,
                                                    element.Value);
